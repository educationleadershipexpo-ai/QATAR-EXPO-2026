    ...
    const validateField = (field: HTMLElement): boolean => {
        ...
        switch (field.id) {
            ...
            case 'form-sponsor-phone':
                ...
                if ((field.id === 'form-booth-phone' || field.id === 'form-sponsor-phone' || field.id === 'form-student-phone') && value === '') {
    ...
    function initializeStudentRegistrationForm() {
        ...
        const otherInterestCheckbox = form.querySelector('#interest-other') as HTMLInputElement;
        const otherInterestWrapper = document.getElementById('other-interest-wrapper');
        const otherInterestText = form.querySelector('#other-interest-text') as HTMLInputElement;

        if (otherInterestCheckbox && otherInterestWrapper && otherInterestText) {
            otherInterestCheckbox.addEventListener('change', () => {
                if (otherInterestCheckbox.checked) {
                    otherInterestWrapper.style.display = 'block';
                } else {
                    otherInterestWrapper.style.display = 'none';
                    otherInterestText.value = '';
                    clearError(otherInterestText);
                }
            });
        }
        ...
        const validateInterestCheckboxes = (): boolean => {
            ...
            let isValid = checkedCheckboxes.length > 0;
            if (isValid) {
            ...
            if (otherInterestCheckbox?.checked) {
                if (otherInterestText.value.trim() === '') {
                    showError(otherInterestText, 'Please specify your area of interest.');
                    isOtherValid = false;
                } else {
                    clearError(otherInterestText);
                }
            }
            return isValid && isOtherValid;
        };
        ...
        if (otherInterestText) {
            otherInterestText.addEventListener('input', validateInterestCheckboxes);
        }
    ...
    function initializeHomePartners() {
        ...
        const partners = [
            ...
            { src: 'https://res.cloudinary.com/dj3vhocuf/image/upload/v1761216928/Blue_Bold_Office_Idea_Logo_50_x_50_px_10_l68ir